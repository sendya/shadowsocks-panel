...
class Smtp implements IMailer
{
    ...
    {
        $this->setReceiver($mail->to);
        $this->setMail($mail->subject, $mail->content);
    ...
    public function __construct()
    {
        ...
        $this->setFrom($this->config['from']); //设置发件人 格式为（ 发件人名称 <xxx@xxx.xx> ）
        $this->setAddress($this->config['smtp_name']); //
        if($this->config['smtp_ssl'] == 'ssl') { //ssl 模式
    ...
    public function sendMail()
    {
        ...
        foreach ($command as $value) {
            $result = $this->_isSecurity ? $this->sendCommandSecurity($value[0],
                $value[1]) : $this->sendCommand($value[0], $value[1]);
            if ($result) {
                continue;
            } else {
                return false;
    ...
    {
        $separator = "----=_Part_" . md5($this->_address . time()) . uniqid(); //分隔符
