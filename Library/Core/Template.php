...
class Template {
	...
	public static function parseUrlRewrite($match){
		...
		if(file_exists(ROOT_PATH.$linkTarget)) return $originText;
	...
	public static function compressCss($match){
		$linkElement = $match[0];
		if (!defined('OPTIMIZE_RES') || !OPTIMIZE_RES) return $linkElement;
		...
		if(!file_exists(ROOT_PATH.$target)) return $linkElement;
