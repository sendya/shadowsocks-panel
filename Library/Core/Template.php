...
class Template {
    ...
    public static function load($templateName) {
        if ($templateName != 'Misc/Error' && $templateName != 'Misc/Redirect') $templateName = TEMPLATE_NAME . "/" . $templateName;
    ...
    private static function compile($templateName) {
        ...
        */
        ...
        $sourceCode = preg_replace_callback('/<link rel="stylesheet" (.+?)>[\r\n]*/is',
            array('\\Core\\Template', 'compressCss'), $sourceCode);

        // Compress JavaScript
        $sourceCode = preg_replace_callback('/<script(.+?)>[\r\n\t ]*<\/script>/is',
            array('\\Core\\Template', 'compressJs'), $sourceCode);
