...
<table>
  ...
  <tr>
    ...
    if(function_exists("mysqli_get_server_info")) {
        $link = mysqli_connect("localhost", "shadowsocks", "shadowsocks");
        $s = @mysqli_get_server_info($link);

        $s = $s ? '&nbsp; mysql_server 版本：'.$s : '';

	    $c = '&nbsp; mysql_client 版本：'.@mysqli_get_client_info($link);
        mysqli_close($link);
        echo $s;
  ...
  ?>
	...
	<table>
		<tr>
			...
			<td class="w_foot"><?php $run_time = sprintf('%0.4f', tz_microtime_float() - $time_start);?>Processed in <?php echo $run_time?> seconds. <?php echo memory_usage();?> memory usage.</td>
